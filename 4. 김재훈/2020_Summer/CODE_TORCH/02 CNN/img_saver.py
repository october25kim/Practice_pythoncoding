...
for idx, (l, d) in enumerate(zip(sample_labels[:train_val_cut_idx], sample_imgs[:train_val_cut_idx])):
    os.makedirs(f'D:/CIFAR10/DATASET/train/train_{idx}', exist_ok=True)
    np.save(f'D:/CIFAR10/DATASET/train/train_{idx}/label.npy', l)
    np.save(f'D:/CIFAR10/DATASET/train/train_{idx}/image.npy', d) 
print('train save finished')


#val save
for idx, (l, d) in enumerate(zip(sample_labels[train_val_cut_idx:], sample_imgs[train_val_cut_idx:])):
    os.makedirs(f'D:/CIFAR10/DATASET/val/val_{idx}', exist_ok=True)
    np.save(f'D:/CIFAR10/DATASET/val/val_{idx}/label.npy', l)
    np.save(f'D:/CIFAR10/DATASET/val/val_{idx}/image.npy', d) 
...
for idx, (l, d) in enumerate(zip(sample_labels[:], sample_imgs[:])):
    os.makedirs(f'D:/CIFAR10/DATASET/test/test_{idx}', exist_ok=True)
    np.save(f'D:/CIFAR10/DATASET/test/test_{idx}/label.npy', l)
    np.save(f'D:/CIFAR10/DATASET/test/test_{idx}/image.npy', d) 
